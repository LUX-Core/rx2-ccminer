...
extern int share_result(int result, int pooln, double sharediff, const char *reason, uint32_t msec);
...
//	headers = curl_slist_append(headers, "User-Agent: " USER_AGENT);
	...
	share_result(res, work->pooln, work->sharediff[0], res ? NULL : (char*) all_data.buf,0);
