...
extern void randomx_init_barrier(const int opt_n_threads);
extern int scanhash_randomx(int thr_id, struct work* work,const uint8_t* seedhash, uint32_t max_nonce, unsigned long *hashes_done);
...
extern void free_randomx(int thr_id);
...
void api_set_workers_per_hash(int thr_id, uint8_t workers_per_hash);
...
void randomx_hash(void *output, const void *input);
