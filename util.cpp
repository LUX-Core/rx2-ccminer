...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *version, *nbits, *stime, *seed;
	...
	bool has_roots = opt_algo == ALGO_RX2 && json_array_size(params) == 11;
	...
	stime = json_string_value(json_array_get(params, p++));
		...
		printf("seed: %s\n",seed);
                ...
                hex2bin(sctx->job.seed,seed,32*sizeof(uint8_t));
		uint8_t temp[32];
		for(int i=0;i<32;i++){
			temp[31-i] = sctx->job.seed[i];
		}
		memcpy(sctx->job.seed,temp,32*sizeof(uint8_t));
//		memset(sctx->job.seed,0,32*sizeof(uint8_t));
        }
