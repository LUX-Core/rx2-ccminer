...
int share_result(int result, int pooln, double sharediff, const char *reason,const uint32_t msec)
{
	const char *flag;
	char suppl[32] = { 0 };
	char solved[16] = { 0 };
	...
	applog(LOG_NOTICE, "accepted: %lu/%lu (%s), latency: %ums %s %s%s",
			p->accepted_count,
			p->accepted_count + p->rejected_count,
			suppl, msec, s, flag, solved);
	if (reason) {
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (pool->type & POOL_STRATUM) {
		...
		else if (opt_algo == ALGO_RX2 && use_roots) {
		...
		if (!share_result(json_is_true(res), work->pooln, work->sharediff[0], reason ? json_string_value(reason) : NULL,0)) {
			if (check_dups)
...
//		for(int i=0;i<32;i++){
//			stratum.job.seed[i] = (uint8_t)rand();
//		}
		return true;
	}
...
static void *miner_thread(void *userdata)
{
	...
	/* Cpu thread affinity */
	if (num_cpus > 1 && opt_algo!=ALGO_RX2) {
			...
			if(thr_id==0){
				if((nonceptr[-1]%32)==0){
					for(int i=0;i<32/4;i++){
						*(uint32_t*)&stratum.job.seed[4*i] = rand();
						printf("%08lX",*(uint32_t*)&stratum.job.seed[i*4]);
					}
					printf("\n");
				}
			}
...
static bool stratum_handle_response(char *buf)
{
	...
	if (stratum.rpc2) {
		...
		share_result(json_is_null(err_val), stratum.pooln, sharediff, reject_reason,stratum.answer_msec);
		if (reject_reason) {
			g_work_time = 0;
			restart_threads();
		}
		...
		share_result(json_is_true(res_val), stratum.pooln, sharediff,
			err_val ? json_string_value(json_array_get(err_val, 1)) : NULL,stratum.answer_msec);
	}
