...
void init_vm(void* entropy_data, void* vm_states, void* num_vm_cycles)
{
	...
	__shared__ uint32_t execution_plan_buf[RANDOMX_PROGRAM_SIZE * 8 * (32 / 8) * sizeof(exec_t) / sizeof(uint32_t)];
...
#	endif
	...
	exec_t* execution_plan = (exec_t*)(execution_plan_buf + (threadIdx.x / 8) * RANDOMX_PROGRAM_SIZE * 8 * sizeof(exec_t) / sizeof(uint32_t));
	...
	if (sub == 0)
	{
		...
		for (uint32_t i = 0; i < RANDOMX_PROGRAM_SIZE; ++i)
		{
			...
			do {
				...
				{
					latency = reg_read_latency;
					update_max(latency, (last_memory_op_slot + 8) / 8);
					is_memory_op = true;
			...
			update_max(first_allowed_slot, latency * 8);
			if (is_cfround)
				update_max(first_allowed_slot, first_allowed_slot_cfround);
			else
				update_max(first_allowed_slot, get_byte(is_fp ? registerReadCycleFP : registerReadCycle, dst) * 8);

			if (is_swap)
				update_max(first_allowed_slot, get_byte(registerReadCycle, src) * 8);
			...
			if (is_fp)
			{
				...
				{
					if ((execution_plan[j] == 0) && (execution_plan[j + 1] == 0) && ((j + 1) &7))
					{
						bool blocked = false;
						for (int32_t k = (j / 8) * 8; k < j; ++k)
						{
			...
			if (is_cfround)
			{
				first_allowed_slot_cfround = slot_to_use - (slot_to_use &7) + 8;
			}
			...
			const uint32_t next_latency = (slot_to_use / 8) + 1;

			if (is_src_read)
			{
				int32_t value = get_byte(registerReadCycle, src);
				update_max(value, slot_to_use / 8);
			...
			else
			{
				...
				if (is_memory_store)
				{
					...
					update_max(value, slot_to_use / 8);
					set_byte(registerReadCycle, dst, value);
					ScratchpadLatency = slot_to_use / 8;
					if ((mod >> 4) >= randomx::StoreL3Condition)
						ScratchpadHighLatency = slot_to_use / 8;
				}
			}
			...
			if (execution_plan[first_available_slot] || (first_available_slot == first_instruction_slot))
			{
				...
				do {
					++first_available_slot;
				} while ((first_available_slot < RANDOMX_PROGRAM_SIZE * 8) && (execution_plan[first_available_slot] != 0));
			}
		...
		atomicAdd((uint32_t*) num_vm_cycles, (last_used_slot / 8) + 1);
		...
		uint32_t datasetOffset = (entropy[13] % (randomx::DatasetExtraItems+1)) * randomx::CacheLineSize;
		...
		for (int32_t i = 0; i <= last_used_slot; ++i)
		{
			...
			while ((i + num_workers <= last_used_slot) && ((i + num_workers) &7) && (execution_plan[i + num_workers] || (i + num_workers == first_instruction_slot) || ((i + num_workers == first_instruction_slot + 1) && first_instruction_fp)))
			{
...
template<int> __device__ uint64_t sqrt_rnd(uint64_t x, uint32_t fprc);
template<> __device__ uint64_t sqrt_rnd<0>(uint64_t a, uint32_t) { return __double_as_longlong(__dsqrt_rn(__longlong_as_double(a))); }
template<> __device__ uint64_t sqrt_rnd<-1>(uint64_t x, uint32_t fprc)
{
	// Check for infinity
	if (x == (2047ULL << 52)) return x;

	const double xd = __longlong_as_double(x);
	// Initial approximation
	double y0, t0, t1;
	asm ("rsqrt.approx.ftz.f64 %0, %1;" : "=d"(y0) : "d"(xd));
	...
	t1 = __fma_rn(-y1_x, y1_x, xd);				// x * (1.0 - x * y1 * y1)

	return __double_as_longlong(fprc==0?__fma_rn(t1,y0,y1_x):fprc==2?__fma_ru(t1,y0,y1_x):__fma_rd(t1,y0,y1_x));
}

template<int> __device__ __forceinline__ uint64_t div_rnd(uint64_t a, uint64_t b, uint32_t fprc);
template<> __device__ __forceinline__ uint64_t div_rnd<0>(uint64_t a, uint64_t b, uint32_t) { return __ddiv_rn(__longlong_as_double(a), __longlong_as_double(b)); }
template<> __device__ __forceinline__ uint64_t div_rnd<-1>(uint64_t a, uint64_t b, uint32_t fprc)
{
	// Check for exact equality
	if (a == b) return 1.0;
	if (a == (2047ULL << 52)) return a;
	const double ad = __longlong_as_double(a);
	const double bd = __longlong_as_double(b);
	// Initial approximation
	double y0;
	asm ("rcp.approx.ftz.f64 %0, %1;" : "=d"(y0) : "d"(bd));

	// First Newton-Raphson iteration
	const double y1 = __fma_rn(y0, __fma_rn(-bd, y0, 1.0), y0);
	const double t0 = ad * y1;
	const double t1 = __fma_rn(-bd, t0, ad);

	// Second Newton-Raphson iteration
	double result = fprc==0?__fma_rn(y1, t1, t0):fprc==2?__fma_ru(y1, t1, t0):__fma_rd(y1, t1, t0);

	// Check for infinity/NaN
	const uint64_t inf_rnd = (2047ULL << 52) - (fprc & 1);

	if (((*((uint64_t*) &result) >> 52) & 2047) == 2047) return inf_rnd;
	return __double_as_longlong(result);
}
...
__device__ __forceinline__ uint32_t inner_loop(const uint32_t program_length,const uint32_t* compiled_program,const int32_t sub,uint8_t* scratchpad,const uint32_t fp_reg_offset,const uint32_t fp_reg_group_A_offset,uint64_t* R,uint32_t* imm_buf,uint32_t fprc,const uint64_t xexponentMask)
{
	...
	for (int32_t ip = 0; ip < program_length;)
	{
		...
		const int32_t num_workers = (inst >> NUM_INSTS_OFFSET) & 7;
		const int32_t num_fp_insts = (inst >> NUM_FP_INSTS_OFFSET) & 7;
		...
		if (sub <= num_workers)
		{
			...
			if (location){
				...
				addr = (addr+static_cast<int32_t>(imm.x)) & mask;
			...
			{
				dst = sqrt_rnd<ROUNDING_MODE>(dst, fprc);
			}
			else if (opcode == 6)
			{
			...
			{
				dst = __mul64hi(static_cast<int64_t>(dst), static_cast<int64_t>(src));
			}
			else if (opcode == 11)
			{
			...
			else if (opcode == 15)
			{
				...
				src = (src & randomx::dynamicMantissaMask) | xexponentMask;
				dst = div_rnd<ROUNDING_MODE>(dst,src, fprc);
			}
			else if (opcode == 5)
			{
...
#define MULT (2)
__global__ void __launch_bounds__(16,20) execute_vm(void* vm_states, void* rounding, void* scratchpads, const void* dataset_ptr)
{
	// 2 hashes per warp, 4 KB shared memory for VM states
	__shared__ uint64_t vm_states_local[(VM_STATE_SIZE * MULT) / sizeof(uint64_t)];

//	load_buffer(vm_states_local, vm_states);
	#pragma unroll
	for(int i=threadIdx.x;i<256*MULT;i+=blockDim.x){
		vm_states_local[i] = ((uint64_t*)vm_states)[blockIdx.x*256*MULT + i];
	}

	uint64_t* R = &vm_states_local[(threadIdx.x / 8) * 256];
	double* F = (double*)(R + 8);
	double* E = (double*)(R + 16);

	const uint32_t thread = blockIdx.x * blockDim.x + threadIdx.x;
	const int32_t idx = thread / 8;
	const int32_t sub = thread & 7;
	...
	uint64_t* p = (((uint64_t*) vm_states) + idx * 256);
	...
	const uint32_t fp_reg_offset = 64 + ((thread & 1) << 3);
	const uint32_t fp_reg_group_A_offset = 192 + ((thread & 1) << 3);
	...
	for (int ic = 0; ic < RANDOMX_PROGRAM_ITERATIONS; ++ic)
	{
		...
		*r ^= *p0;
		...
		fprc = inner_loop<-1>(program_length, compiled_program, sub, scratchpad, fp_reg_offset, fp_reg_group_A_offset, R, imm_buf, fprc, xexponentMask);
